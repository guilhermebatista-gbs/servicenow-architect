...
description: >
  ...
  and legacy scripting, now-sdk CLI/setup). Use when designing, reviewing, building,
  governing, or planning anything on the ServiceNow platform — from "where should this CI
  live in the CMDB?" to "write this Business Rule" to "how do we control scope creep?".
  Routes each task to the right specialized sub-skill. Trigger on: ServiceNow, CMDB, CSDM,
  APM, Business Application, Application Service, Service Mapping, TPM/TRM, governance,
  update set, upgrade, user story, acceptance criteria, scope checkpoint, Fluent SDK,
  .now.ts, GlideRecord, Script Include, Business Rule, Client Script, now-sdk. Bias every
  answer toward business value, product thinking, and platform best practices.
license: MIT
...
| Legacy scripting: GlideRecord, Script Includes, Business Rules, Client Scripts, `g_form`, `g_user`, `gs.*`; server/client boundary | `./sn-scripting/SKILL.md` |
| `now-sdk` CLI, project scaffolding, OAuth/Basic auth profiles, environment configuration | `./sn-sdk-setup/SKILL.md` |
...
2. **Never mix build paradigms.** If the workspace has `now.config.json` or `.now.ts` files, it's a Fluent SDK project — use `sn-sdk-fluent`, not GlideRecord. Otherwise use `sn-scripting`.
